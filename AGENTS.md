...
まず、AndroidアプリのAPKをスマートフォンだけで確認できる開発ループを完成させます。

手順:

1. `mobile/` に最小Androidアプリを作る
2. GitHub ActionsでDebug APKをビルドする
3. APKをGitHub Releaseへ公開する
4. Android端末からGitHubへアクセスする
5. 端末だけでAPKをダウンロードする
6. APKをインストールする
7. アプリが起動し、最小画面が表示されれば合格

この段階ではFirebase / FCM / Backend / 立花証券APIは入れません。

## 5. PoC Stage 1 の最終合格ライン

Android APK確認ループ完成後、次の合格ラインを目指します。
...
1. Android最小アプリ
2. GitHub ActionsでDebug APK生成
3. Android端末でAPKインストール・起動確認
4. Firebase / FCM準備
5. Android端末でPush受信確認
...
- FirebaseやDBを「どうせ後で使うから」という理由で先に入れない
- 問題切り分けを最優先する
...
最小AndroidアプリとGitHub ActionsによるDebug APKビルド設定を作成済みです。
現在は、Android端末からREADME経由で最新APKを取得し、インストール・起動確認できる開発ループの完成を優先します。
